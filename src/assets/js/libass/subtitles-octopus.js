...
var SubtitlesOctopus = function (options) {
  ...
  self.init = function () {
    ...
    self.worker &&
      self.worker.postMessage({
        ...
        URL: document.URL,
        currentScript: self.workerUrl.href
          ? self.workerUrl.href
          : self.workerUrl,
        preMain: true,
  ...
  self.getVideoPosition = function () {
    ...
    var currentStyle =
      self.video.currentStyle || window.getComputedStyle(self.video);
    var x = parseFloat(currentStyle.marginLeft) + (width - realWidth) / 2;
    var y = parseFloat(currentStyle.marginTop) + (height - realHeight) / 2;
  ...
  self.fetchFromWorker = function (workerOptions, onSuccess, onError) {
    try {
      ...
      self.worker && self.worker.postMessage(workerOptions);
    } catch (error) {
      onError(error);
    }
  };
